...
function setup() {
  ...
  reiniciar = createSprite(width/2,height/2+35);
...
function draw() {
  ...
  if (estadoJogo === JOGAR){
    ...
    if(touches.length > 0 || keyWentDown("space") && trex.y >= height-159) {
      trex.velocityY = -12;
    }
...
function gerarNuvens() {
  ...
  if (frameCount % 60 === 0) {
    ...
    nuvem.y = Math.round(random(height/2-20,height-120));
    nuvem.addImage(imagemDaNuvem);
    nuvem.scale = 0.5;
    nuvem.velocityX = -3;
    
     //atribuir tempo de duração à variável
    nuvem.lifetime = width/2 + 200; 
...
function gerarObstaculos() {
  if(frameCount % 60 === 0) {
    var obstaculo = createSprite(width,height-45,10,40);
    ...
    obstaculo.scale = 0.5;
    obstaculo.lifetime = width/2 + 200;
